...
FROM
  Products
      JOIN
  ProductVariations ON Products.id = ProductVariations.productId
      JOIN
  ProductProperties ON ProductVariations.id = ProductProperties.productVariationId
      JOIN
  Pictures ON Products.id = Pictures.pictureableId
...
const getProductByStoreId = async ({ storeId }) => {
  const products = await models.Product.findAll({
    ...
    include: [
      { model: models.Picture },
      { model: models.ProductVariation, include: [{ model: models.Picture }, { model: models.ProductProperty }] },
    ],
  });
  if (products.length > 0) {
    return products;
