...
const { createAddress, updateAddress, deleteAddress } = require('../services');
...
const addAddress = async (req, res, next) => {
    try {
      ...
      return response.successWithMessage("address created successfully", res,Address);
  } catch (err) {
  ...
  try {
    ...
    if (id === Address.addressableId) {
      ...
      if (city) {
        ...
        return response.failedWithMessage("Address not found!", res);
      }
      return response.failedWithMessage("City not found!", res);
    } else {
      return response.failedWithMessage("You can't update this address!", res);
    }
  } catch (err) {
    console.log("ERROR--> ", err);
    return response.serverError(req, res);
...
const destroy = async (req, res, next) => {
  try {
    ...
    if (id === Address.addressableId) {
      ...
      return response.failedWithMessage("You can't delete this address!", res);
    }
  } catch (err) {
    console.log("ERROR--> ", err);
    return response.serverError(req, res);
