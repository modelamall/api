...
const { getAddressByAddressableId } = require("../../address/services/index");
...
const deleteAdmin = async (req, res, next) => {
  ...
  const addresses = await getAddressByAddressableId(admin.id);
  if (admin) {
    const deleted = await adminServices.destroyAdmin({admin,addresses});
    if (deleted) {
