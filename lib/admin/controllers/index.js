...
const update = async (req, res, next) => {
  try {
    ...
    const update = await adminServices.updateAdmin({
      id: req.user.id,
      name,
      username,
      email,
    ...
    if (update) {
      const newAdmin = await getInstanceById(req.user.id, "Admin")
      const transformeredAdmin = transformer.adminTransformer(newAdmin);
      return response.successWithMessage("Updated successfully", res, transformeredAdmin);
    } else
      return response.failedWithMessage(
