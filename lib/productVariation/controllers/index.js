...
const deleteProductVariation = async (req, res, next) => {
  ...
  const deleted = await productVariationServices.destroyProductVariation({ productVariation });
  if (deleted) {
    return response.successWithMessage("Product Variation deleted successfully", res);
  }
  return response.serverError(req, res);
};
